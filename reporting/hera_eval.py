...
from models import Encoder, Autoencoder, Discriminator
    ...
    for model_type in models:
        ...
        for ld in list(pd.unique(df_.Latent_Dim)):
            ...
            for i, clss in enumerate(list(pd.unique(df.Class))):
                ...
                ae = Autoencoder(args)
                p = 'outputs/{}/{}/{}/training_checkpoints/checkpoint_full_model_ae'.format(model_type, clss,
                                                                                            model_name)
                ae.load_weights(p)

                if model_type == 'GANomaly':
                    encoder = tf.keras.Sequential(Encoder(args))
                    p = 'outputs/{}/{}/{}/training_checkpoints/checkpoint_full_model_encoder'.format(model_type, clss,
                                                                                                     model_name)
                    ...
                    p = 'outputs/{}/{}/{}/training_checkpoints/checkpoint_full_model_disc'.format(model_type, clss,
                                                                                                  model_name)
                    disc.load_weights(p)
                    model = [ae, disc]
                ...
                for nneigh in args.neighbours:
                    ...
                    dists = get_dists(neighbours_dist, args.raw_input_shape, args.patch_x, args.patch_y)
                    ...
                    for alpha in [0.00, 0.25, 0.50, 0.75, 1.00]:
                        ...
                        df_out.loc[indxer] = (
                        model_type, args.anomaly_class, ld, nneigh, alpha, detections[-1], lofs[-1], ifs[-1], svms[-1])
                        indxer += 1
...
def get_dists(neighbours_dist, args):
    ...
    if args.patches:
        ...
        dists_recon = reconstruct(np.expand_dims(dists, axis=-1), args.raw_input_shape, args.patch_x, args.patch_y)
        return dists_recon
    else:
        return dists
