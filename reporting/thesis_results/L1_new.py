...
def common_L1_kwargs(min_f1=0.5, task='train'):
    ...
    kwargs = dict(
        ...
        save_name=f'dummy',
        save_path=f'/home/ee487519/PycharmProjects/RFI-NLN/reporting/thesis_results/L1_new',
    )
    return kwargs
...
generic_gmm(groupby=['loss'], incl_models=[], query_strings=['reg_label == "l2+0.1"',],
            x_axis='val_f1',
            y_axis='test_f1', xlabel='Validation F1', ylabel='Test F1', line_ellipse=le,
            fontsize=55,
            figsize=(20, 15),
            legendspacing=0.0,
            legend_borderpad=0,
...
generic_gmm(groupby=['short_model_class'], incl_models=[], query_strings=['reg_label == "l2+0.1"', 'loss == "dice"'],
            ...
            ylim_bottom=.64,
            ylim_top=.88,
            save_name='eachmodels_bestreg_bestloss_MNRAS')
exit()
