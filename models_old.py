...
def AC_UNET_conv2D_blocks(input_tensor, n_blocks, n_filters, kernel_size=3, stride=1, dilation_rate=3, dropout=0.0,
                      batchnorm=True):
    x = input_tensor
    for i in range(n_blocks):
...
def AC_UNET(args, n_filters=64, dilation_rate=7, dropout=0.05, batchnorm=True):
    ...
    d1 = AC_UNET_conv2D_blocks(input_data, 2, n_filters, dilation_rate=dilation_rate, batchnorm=batchnorm, dropout=dropout)
    m1 = layers.MaxPooling2D(pool_size=2,strides=2,padding='valid')(d1)

    d2 = AC_UNET_conv2D_blocks(m1, 2, n_filters*2, dilation_rate=dilation_rate, batchnorm=batchnorm, dropout=dropout)
    m2 = layers.MaxPooling2D(pool_size=2,strides=2,padding='valid')(d2)

    # x is the center layers in the belly of the U
    x = AC_UNET_conv2D_blocks(m2, 2, n_filters*4, dilation_rate=dilation_rate, batchnorm=batchnorm, dropout=dropout)
    u2 = layers.Conv2DTranspose(n_filters*2, 3, strides=2, padding='same')(x)
    x = layers.concatenate([u2, d2])

    x = AC_UNET_conv2D_blocks(x, 2, n_filters*2, dilation_rate=dilation_rate, batchnorm=batchnorm, dropout=dropout)
    u1 = layers.Conv2DTranspose(n_filters, 3, strides=2, padding='same')(x)
    x = layers.concatenate([u1, d1])

    x = AC_UNET_conv2D_blocks(x, 2, n_filters, dilation_rate=dilation_rate, batchnorm=batchnorm, dropout=dropout)
...
if __name__ == '____':
    ...
    m = AC_UNET(Args)
    a = np.random.random((123, 100, 100,1))
    out = m(a)
    print(out.shape)
