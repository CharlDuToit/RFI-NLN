#!/bin/bash
echo "Logging for run_hera_charl_aof.sh at time: $(date)." >> hera_charl_aof.log
...
do
		...
		python3 -u main.py \
          ...
          -percentage_anomaly $percentage\
				  ...
				  -output_path ./outputs\
				  -save_dataset False\
				  -shuffle_patches $shuffle_patches > >(tee -a hera_charl_aof.log) 2>&1
		done 
done

echo "Completed run_hera_charl_aof.sh at time: $(date)." >> hera_charl_aof.log
