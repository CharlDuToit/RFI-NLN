...
def find_thresholds_and_averages(thr_lists, x_lists, y_lists):
    ...
    thr_lists = [[1.0] + lst[0:le] for lst, le in zip(thr_lists, min_lens)]
    x_lists = [[0.0] + lst[0:le] for lst, le in zip(x_lists, min_lens)]
    y_lists = [[1.0] + lst[0:le] for lst, le in zip(y_lists, min_lens)]
    # thr_lists = [[1.0] + lst[0:le] for lst, le in zip(thr_lists, min_lens)]
    # x_lists = [[0.0] + lst[0:le] for lst, le in zip(x_lists, min_lens)]
    # y_lists = [[1.0] + lst[0:le] for lst, le in zip(y_lists, min_lens)]
...
def save_recall_prec_curve(df: pd.DataFrame,
                           groupby: Tuple[str, ...],
                           #x_axis: str,
                           #y_axis: str,
                           aof_score='None', # 'LOFAR', 'HERA_CHARL'
                           gmm_thresholds=(0.5,),
                           f1_contours=(0.0, ),
                           val_and_test=False,
                           data_subset='test',
                           ...
                           xlim_top=1.0, xlim_bottom=0.0, ylim_top=1.0, ylim_bottom=0.0,
                           dir_path='./', show=False, show_legend=False, logx=False, logy=False, xlabel='Recall',
                           ylabel='Precision',
    ...
    # ---------------------------------------------------
    # Countours
    recall_range = np.linspace(xlim_bottom, xlim_top, 100)
    precision_range = np.linspace(ylim_bottom, ylim_top, 100)
    recall_mesh, precision_mesh = np.meshgrid(recall_range, precision_range)
    f1_values = calculate_f1(recall_mesh, precision_mesh)
    contour_levels = list(f1_contours)
    ...
    for i, (group_keys, group) in enumerate(grouped):
        ...
        if not val_and_test:
            recall_vals = group[f'{data_subset}_recall_vals']
            prec_vals = group[f'{data_subset}_prec_vals']
            thr_vals = group[f'{data_subset}_prec_recall_thr_vals']
        else:
            ds = group_keys[-1]
            recall_vals = group[f'{ds}_recall_vals']
            prec_vals = group[f'{ds}_prec_vals']
            thr_vals = group[f'{ds}_prec_recall_thr_vals']


        thr_vals, recall_vals, prec_vals, mean_recall_vals, mean_prec_vals = find_thresholds_and_averages(
            [a for a in thr_vals],
            [a for a in recall_vals],
            [a for a in prec_vals],
        )
        ...
        # ---------------------------------------------------
        # Plot average curve
        ax.plot(mean_recall_vals, mean_prec_vals,
                color=color,
                linewidth=linewidth,
                linestyle=linestyle,)
        ...
        if scatter_thresholds:
            ax.scatter(mean_recall_vals, mean_prec_vals,
                       color=color,
                       # label=group_keys[0],
                       s=size,
                       marker=marker,)
        ...
        if aof_score in ('LOFAR', 'HERA_CHARL'):
            if aof_score == 'LOFAR':
                rec, prec = 0.58019, 0.5598 # F1 = 0. 5698
            if aof_score == 'HERA_CHARL':
                rec, prec = 0.8635519709326119, 0.4792665150114434
            ...
            ax.scatter(rec, prec,
                       ...
                       marker='o', )
            ax.annotate('AOF',
                        (rec, prec),  # Position for bar value
                        fontsize=legend_fontsize,
                        ha='center', va='center',
                        xytext=(0, 20),
                        textcoords='offset points')
            ax.plot((rec, rec), (ylim_bottom, ylim_top), color='black', linewidth=linewidth, linestyle='--')
            ax.plot((xlim_bottom, xlim_top), (prec, prec), color='black', linewidth=linewidth, linestyle='--')

        # ---------------------------------------------------
        # Fit Gaussian Mixture Model to the group at specified thresholds
        if scatter_gmm_means or scatter_gmm_points:
            # if group_keys == 'mse': gmm_thresholds = (0.1, 0.5, 0.9)
            for thr in gmm_thresholds:
                idx = np.argmin(np.abs(thr - thr_vals))
                gmm_recall_vals = recall_vals[idx]
                gmm_prec_vals = prec_vals[idx]
                mean_recall = mean_recall_vals[idx]
                mean_prec= mean_prec_vals[idx]

                # if group_keys == 'R5' and thr==1e-5: print('prec=', mean_prec, ' recall=', mean_recall)

                # Plotting the mean point with color and line type
                if scatter_gmm_means:
                    ax.scatter(mean_recall, mean_prec,
                               color=color,
                               # label=group_keys[0],
                               s=size * mean_size_factor,
                ...
                if scatter_gmm_points:
                    ax.scatter(gmm_recall_vals, gmm_prec_vals,
                               color=color,
                               s=size,
                               marker=marker,
                               )

                if len(gmm_recall_vals) > 1 and line_ellipse:

                    gmm = GaussianMixture(n_components=1)
                    gmm.fit(np.vstack([gmm_recall_vals, gmm_prec_vals]).T)
...
if __name__ == '__main__':
    ...
    save_recall_prec_curve(df, ['Group1', 'Group2', 'Group3'], 'X', 'Y', size=50,
