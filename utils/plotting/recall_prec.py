...
def save_recall_prec_curve(df: pd.DataFrame,
    ...
    contour_levels = sorted(list(np.unique( list(contour_levels) + list(f1_contours))))
    ...
    for i, (group_keys, group) in enumerate(grouped):
        ...
        # -----
        recall_lens = [len(a) for a in recall_vals]
        unique_lens = np.unique(recall_lens)
        print('Recall Array Lengths for ', group_keys, ' : ', unique_lens)

        if len(unique_lens) == 1:
            recall_vals = np.array([a for a in recall_vals])
            prec_vals = np.array([a for a in prec_vals])
            ...
            if same_first_thr and same_last_thr:
                ...
                recall_vals = np.array([resample_linear_1d(a, new_length) for a in recall_vals])
                prec_vals = np.array([resample_linear_1d(a, new_length) for a in prec_vals])
        ...
        mean_recall_vals = recall_vals.mean(axis=0)
        mean_prec_vals = prec_vals.mean(axis=0)
        ...
        enough_points = recall_vals.shape[0] > 1
        ...
        if enough_points and (scatter_gmm_means or scatter_gmm_points):
            for thr in gmm_thresholds:
                ...
                # np.vstack([gmm_recall_vals, gmm_prec_vals]).T

                gmm = GaussianMixture(n_components=1)
                # gmm.fit(group[[x_axis, y_axis]])
                # gmm.fit([gmm_recall_vals, gmm_prec_vals])
